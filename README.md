...
- 무한 스트롤 시 API 중복 호출 <br />
→ 최상단에서 api를 호출하는 렌더링 구조 (부모 → 자식 → 렌더링 반복 호출)

- 캐러셀 휠 넘김 속도 과다 <br />
→ debounce/throttle 미적용 추정

- TextField 컴포넌트에서 dispatch 사용 방식에 대한 의문 <br />
→ 더 적절한 상태 처리 방식 검토 필요

- input 커스텀 훅의 대문자 변환 처리 <br />
→ SET_(대문자) 형식 미사용 시 오류 가능성, 처리 방식 개선 필요

- 기술 면접 대비 스스로 너무 부족하다고 판단 <br />
→ 노션에 따로 질문 받은 것들 포함하여 개념 정리
